...
```mermaid
flowchart LR
    User([End User]) -->|HTTPS| UI[React SPA<br/>Tailwind + Material UI + Axios + Chart.js]
    UI -->|REST / JSON| API[Spring Boot REST API<br/>Controllers & Services]
    API -->|Spring Data JPA| MySQL[(MySQL Schema<br/>`departments`, `employees`, `users`)]
...
flowchart LR
    Dev[Git Changes]
    Jenkins[Jenkins Pipeline<br/>`npm install` + `npm run build`]
    Makefile[Makefile & scripts<br/>Docker/k8s helpers]
    Images[Docker Images<br/>frontend & backend]
    Infra[Terraform AWS Stack<br/>EKS · RDS · ECR]
...
flowchart LR
    ...
    K8s -->|REST / JSON| API[Spring Boot REST API<br/>Controllers & Services]
    API -->|Spring Data JPA| MySQL[(MySQL Schema<br/>`departments`, `employees`, `users`)]
    API -->|Optional Spring Data Mongo| Mongo[(MongoDB Cluster)]
    User([End User]) -->|HTTPS| UI[React SPA<br/>Tailwind + Material UI + Axios + Chart.js]
    UI -->|REST / JSON| API
